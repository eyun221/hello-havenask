#ifndef AUTIL_DATABUFFER_H
#define AUTIL_DATABUFFER_H
...
// ATTENTION:
// 1. POD may not be serialized as simple type (pointer member in class)
// 2. tr1::is_pod implementation is not complete
// Maybe not use raw pointer in the future
...
public:
    ...
    DataBuffer(void* data, size_t dataLen, autil::mem_pool::Pool *pool = NULL) {
...
private:
    ...
    void init(int len, autil::mem_pool::Pool *pool);
    void init(void* data, size_t dataLen, autil::mem_pool::Pool *pool);
    void reset();
...
SERIALIZE_PTR(std::tr1::shared_ptr)
