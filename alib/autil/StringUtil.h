#ifndef AUTIL_STRINGUTIL_H
#define AUTIL_STRINGUTIL_H
...
template<>
inline std::string StringUtil::toString<autil::HllCtx>(const autil::HllCtx &x) {
    uint64_t count = autil::Hyperloglog::hllCtxCount(&x);
    StringStreamWrapper oss;
    oss << count;
    return oss.str();
}
