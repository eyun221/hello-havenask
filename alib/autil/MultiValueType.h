#ifndef AUTIL_MULTIVALUETYPE_H
#define AUTIL_MULTIVALUETYPE_H
...
template <>
inline void MultiValueType<MultiValueType<char>>::serialize(autil::DataBuffer& dataBuffer) const {
    uint32_t len = length();
    dataBuffer.write(len);
    if (len > 0) {
        const char* data = getBaseAddress();
        if (len == 2u && (*data) == '\0') {
            // see https://work.aone.alibaba-inc.com/issue/23705044
            uint16_t zero = 0;
...
#define SPECIALIZE_MULTIVALUETYPE_DATABUFFER_READ(type) \
    template<>                                          \
    inline void DataBuffer::read(type &value) {         \
        value.deserialize(*this, _pool);                \
    }
MULTI_VALUE_TYPE_MACRO_HELPER(SPECIALIZE_MULTIVALUETYPE_DATABUFFER_READ);
#undef SPECIALIZE_MULTIVALUETYPE_DATABUFFER_READ
