...
protected:
    ...
    struct ThreadCounter : public AlignedThreadData {
        ...
        ThreadCounter(std::atomic_uint_fast64_t* _mergedSum,
                      std::atomic_uint_fast64_t* _mergedNum)
            : sum(0)
            , num(0)
...
public:
    ...
    GaugeRecorder(Monitor* monitor, const std::string& metricName,
...
protected:
    AlignedThreadData* createThreadData() override {
        return new ThreadCounter(&_mergedSum, &_mergedNum);
    }
