 ...
 * modify:
    ...
    2. 2020-02-06 depart metric and tags to improve perf, refer issue: http://gitlab.alibaba-inc.com/monitor_service/kmonitor-client-cpp/issues/86028
 * */
...
class KMonitor : public MetricsSource {
 public:
    ...
    void SetConfig(MetricsConfig *config);
    ...
    void Register(const std::string &name, MetricType metric_type, int statistics_type, MetricLevel level = NORMAL);
    void Unregister(const std::string &name);

    MutableMetric* RegisterMetric(const std::string &name, MetricType type, MetricLevel level = NORMAL);
    MutableMetric* RegisterMetric(const std::string &name, MetricType metric_type, int statistics_type,
    ...
    void Stop();
    void GetMetrics(MetricsCollector *collector, const std::set<MetricLevel>& levels, bool all, int64_t curTime);
