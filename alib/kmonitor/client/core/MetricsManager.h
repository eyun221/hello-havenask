...
class MetricsManager {
 ...
 public:
    void Snapshot(MetricsCollector* collector, const std::set<MetricLevel>& levels, bool all, int64_t curTime);
    void Register(const std::string &metric_name, MetricType type, int statistic_type, MetricLevel level, MetricsTagsManager* tags_manager, MetricsCache *metricsCache = NULL);
    MutableMetric* RegisterMetric(const std::string &metric_name, MetricType type, int statistic_type, MetricLevel level, MetricsTagsManager* tags_manager, MetricsCache *metricsCache = NULL);
